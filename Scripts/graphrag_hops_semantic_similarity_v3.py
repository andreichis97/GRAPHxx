...
Run with the existing .venv, for example:
    python graphrag_hops_semantic_similarity_v3.py --repository fictional_companies --hops 2
        --question "Who manages Ana Popescu?" --graphdb-url http://localhost:7200

Request files use one field per line: repository, question, hops, optional chat
model, optional GraphDB base URL. Blank optional fields use Config defaults.
...
class Config:
    ...
    def __post_init__(self):
        base_url = self.graphdb_url.strip().rstrip("/")
        ...
        object.__setattr__(self, "graphdb_url", base_url)
        if self.repository_url is None:
            object.__setattr__(self, "repository_url",
                               f"{base_url}/repositories/fictional_companies")
...
def main() -> int:
    ...
    parser.add_argument("--request-file", help="UTF-8 file: repository, question, hops, optional chat model, optional GraphDB base URL on separate lines. Non-empty fields override CLI arguments.")
    ...
    repository_args.add_argument(
        "--repository", help="Repository name, e.g. caise_graphs or ace_company.")
    repository_args.add_argument(
        "--repository-url", help="Full repository URL for a custom server.")
    parser.add_argument("--graphdb-url", default=Config.graphdb_url,
    ...
    if args.request_file:
        ...
        if any(line.strip() for line in lines[5:]):
            parser.error("Request files support five fields; keep the question on line 2.")
        lines = (lines + [""] * 5)[:5]
        repository_name, question, hops, chat_model, graphdb_url = (
            line.strip() for line in lines)
        if not repository_name or not question:
            parser.error("Enter the repository on line 1 and a non-empty question on line 2.")
        ...
        if hops:
            try:
                ...
                parser.error("Hops on line 3 must be a positive integer.")
        if chat_model:
            args.chat_model = chat_model
        if graphdb_url:
            args.graphdb_url = graphdb_url
    if args.repository is not None:
        ...
        args.repository_url = f"{args.graphdb_url.strip().rstrip('/')}/repositories/{args.repository}"
    elif args.repository_url is None:
        args.repository_url = os.getenv("GRAPHRAG_REPOSITORY_URL")
